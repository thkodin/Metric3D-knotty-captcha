# default dataset settings - ensure you add these to relevant training/mono/configs/RAFTDecoder/ config(s).
# data will resized/cropped to the canonical size, refer to ._data_base_.py

# NOTE-TAIMOOR: This was created by using nyu.py and kitti.py as references, adjusted for a render of the knotty captcha dataset
# (different render sets might need adjustments depending on Blender render settings in the knotty-captcha project).
knotty_captcha_dataset = dict(
    lib="KnottyCaptchaDataset",  # NOTE-TAIMOOR: the name of the dataset class exposed in training/mono/datasets/__init__.py
