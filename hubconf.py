...
def prepare_data(rgb_file: Path, depth_file: Path, gt_depth_scale: float) -> tuple[np.ndarray, np.ndarray]:
    """
    Load and prepare RGB and depth data from files.

    Args:
        rgb_file (Path): Path to the RGB image file.
        depth_file (Path): Path to the depth image file. Can be None if depth is not required.
        gt_depth_scale (float): Scale factor to convert depth values to metric units.
    ...
    if depth_file is not None:
        gt_depth = cv2.imread(str(depth_file), -1) / gt_depth_scale
...
if __name__ == "__main__":
    DIR_INFERENCE_DATA = Path("inference")
